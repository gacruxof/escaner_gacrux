...
<head>
    ...
    <style>
        ...
        #btn-scan { width: 100%; padding: 15px; font-size: 1.2em; font-weight: bold; color: white; background: #1e3a8a; border: none; border-radius: 8px; cursor: pointer; transition: 0.2s; box-shadow: 0 4px 0 #11111b; margin-bottom: 15px;}
        #btn-scan:active { transform: translateY(4px); box-shadow: 0 0 0 #11111b; }
        .btn-scanning { background: #d08c00 !important; }
        
        select { width: 100%; padding: 10px; font-size: 1em; border-radius: 6px; border: 1px solid #45475a; background: #313244; color: white; font-weight: bold; outline: none;}
        
        #status { margin-top: 15px; font-weight: bold; color: #a6e3a1; font-size: 1.1em; }
    </style>
</head>
<body>
    <h2>🚀 GACRUX SCANNER</h2>
    
    <!-- El lector de la cámara -->
    <div id="reader"></div>
    ...
    <script>
        ...
        function activarEscaneo() {
            if (audioCtx.state === 'suspended') audioCtx.resume(); // Desbloquear audio
            ...
            statusDiv.innerText = "Cámara lista. Pasa el código de barras.";
            statusDiv.style.color = "#89b4fa";
        }

        // Función que se ejecuta internamente cada vez que la cámara ve un código
        function onScanSuccess(decodedText, decodedResult) {
            // Si no le hemos dado al botón, ignorar el código
            if (!listoParaEscanear) return;
            
            // Si pasamos el filtro, apagamos el escáner para no duplicar
            listoParaEscanear = false; 
            
            // Reproducir sonido matemático
            playSound();
            ...
            .then(data => {
                ...
                setTimeout(() => { btnScan.innerText = "📷 ESCANEAR OTRO"; }, 1000);
            })
            .catch(err => {
                statusDiv.innerText = "❌ Error de conexión al enviar";
                statusDiv.style.color = "#f38ba8";
                setTimeout(() => { btnScan.innerText = "🔁 INTENTAR DE NUEVO"; }, 1500);
            });
        }

        // Encender la cámara (estará prendida pero ignorando códigos hasta darle al botón)
